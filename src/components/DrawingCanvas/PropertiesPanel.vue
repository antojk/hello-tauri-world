  ...
  <div class="h-full overflow-y-auto p-4">
    ...
    <div class="mb-6">
      ...
      <div class="flex justify-between items-center mb-2">
        ...
        <div class="flex gap-1">
          <button v-for="unit in measurementUnits" :key="unit"
            class="px-2 py-1 text-xs rounded border transition-colors duration-200" :class="[
              gridSettings.unit === unit
                ? 'bg-primary-500 text-white border-primary-600'
                : 'bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600'
      ...
      <div class="flex justify-between items-center mb-2">
        ...
        <input type="number" min="1" max="100" step="1" :value="gridSettings.gridSize" @change="changeGridSize" class="w-20 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 
                 bg-white dark:bg-gray-800 text-right" />
      </div>

      <div class="flex justify-between items-center mb-2">
        <label class="font-medium">Show Grid:</label>
        <input type="checkbox" :checked="gridSettings.showGrid" @change="toggleGrid"
          class="h-4 w-4 text-primary-600 rounded-sm" />
      </div>

      <div class="flex justify-between items-center mb-2">
        <label class="font-medium">Snap to Grid:</label>
        <input type="checkbox" :checked="gridSettings.snapToGrid" @change="toggleSnapToGrid"
          class="h-4 w-4 text-primary-600 rounded-sm" />
      </div>
    </div>
    ...
    <div class="mb-6">
      ...
      <template v-else-if="currentShapeType === 'polygon' && isPolygon(currentShape) && polygonDimensions">
        ...
        <div v-for="(point, index) in currentShape.points" :key="index" class="flex justify-between items-center mb-2">
